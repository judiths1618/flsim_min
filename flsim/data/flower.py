...
# === Added helpers for array partitions (safe if flwr-datasets is available) ===
def load_flower_arrays(
    ...
    """Return dicts of X[cid]->[N,D] and y[cid]->[N] plus (D, K).
    - Works for simple image/row datasets in flwr-datasets if installed
    - Falls back to ImportError if package not present
    """
    if FederatedDataset is None:
        raise ImportError("flwr-datasets is not installed. Install with extras: `pip install .[data]`")

    # FIX: DirichletPartitioner 需要 partition_by；同时给两个 partitioner 都加 seed
    if iid:
        # part = IidPartitioner(num_partitions=int(n_clients), seed=seed)
        part = IidPartitioner(num_partitions=int(n_clients))
    else:
        part = DirichletPartitioner(
            num_partitions=int(n_clients),
            partition_by="label",   # 关键修复
            alpha=float(alpha),
            seed=seed,
        )
    ...
    for cid in range(n_clients):
        ...
        Xp[int(cid)] = X
        yp[int(cid)] = y
        D = int(X.shape[1]) if X.size > 0 else D
        K = max(K, int(np.max(y) + 1) if y.size > 0 else K)
